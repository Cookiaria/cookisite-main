...
let suffixes = [
  ...
  "^-^",
  "x3",
  ["rawr", "rawr~", "rawr~~", "rawr x3", "rawr~ x3", "rawr~~ x3"],
  ["owo", "owo~", "owo~~"],
  ["uwu", "uwu~", "uwu~~"],
  "-.-",
  ">w<",
  ":3",
  ["nya", "nya~", "nya~~", "nyaa", "nyaa~", "nyaa~~"],
  [">_<", ">-<"],
  ":flushed:",
  "👉👈",
  ["^^", "^^;;"],
...
let replacements = [
  ...
  [
    ...
    (_escape, isIgnoredAt) =>
      function (match, offset, string) {
        ...
        if (!getChance(settings.periodToExclamationChance)) return match;
        return "!";
      },
  ],
      ...
      function (match, offset, string) {
        ...
        if (getChance(settings.duplicateCharactersChance)) {
          let amount = Math.floor(
            (Math.random() + 1) * (settings.duplicateCharactersAmount - 1),
          );
          let newMatch = match;
          for (let i = 0; i < amount; i++) newMatch += match;
          match = newMatch;
        }
        return match;
      },
        ...
        for (const [pattern, replacement] of simpleReplacements) {
          ...
          if (
            (pattern instanceof RegExp &&
              match.match(pattern)?.includes(match)) ||
            pattern == match
          )
            continue;
  ...
  [
    ...
    (_escape, isIgnoredAt) =>
      function (match, offset, string) {
        if (isIgnoredAt(offset, string)) return match;
        if (!getChance(settings.stutterChance)) return match;
        return `${match}-${match}`;
      },
  ],
  // suffixes
